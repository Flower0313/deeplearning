...
def main():
    client.start()
    # 频道实体
    entity = client.get_entity(Channel(id=1647055020, title='上海楼凤公众号',
                                       photo=ChatPhoto(photo_id=5026198226063174277, dc_id=1, has_video=False,
                                                       stripped_thumb=''),
                                       date=datetime.datetime(2023, 5, 30, 7, 28, 10, tzinfo=datetime.timezone.utc),
                                       ...
                                       join_request=False, forum=False, access_hash=-1876271609375836813,
                                       username='shmianfei', restriction_reason=[], admin_rights=None,
                                       banned_rights=None, default_banned_rights=None, participants_count=5781,
                                       usernames=[]))
    ...
    for message in messages:
        ...
        if message.photo:
            ...
            if str(message.message) != '' and message.message is not None and re.search(r"\d{6,8}",
                                                                                        message.message) and str(message.message).__contains__("@"):
